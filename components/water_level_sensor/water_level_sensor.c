...
esp_err_t wls_init(wls_config_t *wls_config, wls_handle_t *wls_handle)
{
    esp_err_t ret = ESP_OK;

    int io_num = wls_config->pin;
    adc_unit_t unit_id;
    ...
    wls_handle->adc_unit_handle = adc_handle;
    wls_handle->adc_channel = channel;
    wls_handle->is_calibrated = calibrated;
    wls_handle->adc_cali_handle = adc_cali_handle;
...
esp_err_t wls_read(wls_handle_t *wls_handle, wls_data_t *wls_data)
{
    esp_err_t ret = ESP_OK;

    adc_oneshot_unit_handle_t handle = wls_handle->adc_unit_handle;
    adc_channel_t chan = wls_handle->adc_channel;
    int out_raw;
    ESP_ERR_CHECK_LOG_GOTO(adc_oneshot_read(handle, chan, &out_raw), "adc_oneshot_read", error);
    wls_data->raw = out_raw;

    wls_data->calibrated = -1;
    if (wls_handle->is_calibrated)
    {
        adc_cali_handle_t cali_handle = wls_handle->adc_cali_handle;
        int cali_result;
        ESP_ERR_CHECK_LOG_GOTO(
            adc_cali_raw_to_voltage(cali_handle, out_raw, &cali_result),
            "adc_cali_raw_to_voltage",
            error);
        wls_data->calibrated = cali_result;
    }
    return ret;
error:
...
esp_err_t wls_deinit(wls_handle_t *wls_handle)
{
    esp_err_t ret = ESP_OK;
    /*struct wls_handle_s
    {
    ...
    ESP_ERR_CHECK_LOG_GOTO(adc_oneshot_del_unit(wls_handle->adc_unit_handle) | adc_cali_delete_scheme_line_fitting(wls_handle->adc_cali_handle), "something went wrong", error);
    return ret;
error:
    ESP_LOGE(TAG, "something went wrong in wls_deinit() : %s", esp_err_to_name(ret));
    return ret;
}